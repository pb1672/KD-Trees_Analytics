...
def naiveApproach(tripLocations, startRectangle, endRectangle):
    ...
    for a in tripLocations:
        if startRectangle[0][0]<=a[0] and startRectangle[0][1]>=a[0] and startRectangle[1][0]<=a[1] and startRectangle[1][1]>=a[1] and endRectangle[0][0]<=a[2] and endRectangle[0][1]>=a[2] and endRectangle[1][0]<=a[3] and endRectangle[1][1]>=a[3]:
            indices.append(num)
        num+=1
    print indices
...
def kdtreeApproach(tripLocations, startRectangle, endRectangle):
    ...
    s = sqrt((startRectangle[0][0] - startRectangle[0][1])**2 + (startRectangle[1][0] - startRectangle[1][1])**2)/float(2)
    e = sqrt((endRectangle[0][0] - endRectangle[0][1])**2 + (endRectangle[1][0] - endRectangle[1][1])**2)/float(2)
    ...
    tree1 = KDTree(trip1)
    tree2 = KDTree(trip2)
    start = set(tree1.query_ball_point([startx,starty], s))
    end = set(tree2.query_ball_point([endx, endy], e))
    in_both = list(start.intersection(end))
    print len(in_both)
    for a in in_both:
        if startRectangle[0][0]<=tripLocations[a][0] and startRectangle[0][1]>=tripLocations[a][0] and startRectangle[1][0]<=tripLocations[a][1] and startRectangle[1][1]>=tripLocations[a][1] and endRectangle[0][0]<=tripLocations[a][2] and endRectangle[0][1]>=tripLocations[a][2] and endRectangle[1][0]<=tripLocations[a][3] and endRectangle[1][1]>=tripLocations[a][3]:
            indices.append(i)
    print indices        
